...
linked GST registration has already been fully PAID but the lead never moved
out of SCHEDULED_PAYMENTS / GST_REGISTRATION_DONE — i.e. payments completed
*before* the payment-completion auto-sync landed.
...
SUBSCRIBED), identical to a live payment completion. Forward-only; leads already
SUBSCRIBED / NOT_INTERESTED are left untouched. Idempotent.
...
# GST funnel stages that a completed payment should push to SUBSCRIBED.
_PAID_FROM_STAGES = ("GST_REGISTRATION_DONE", "SCHEDULED_PAYMENTS")
...
async def main() -> None:
    ...
    async with pool.acquire() as conn:
        entity_rows = await conn.fetch(
            ...
            """,
            GST_CRM_ENTITY_TYPE,
            list(_PAID_FROM_STAGES),
        )
        print(
            f"Found {len(entity_rows)} fully-PAID GST entity(ies) "
        ...
        for row in entity_rows:
            ...
            for lid in ids:
                print(
                    f"  entity_id={entity_id} -> lead_id={lid} "
